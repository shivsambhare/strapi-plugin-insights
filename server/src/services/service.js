...
const TEXT_FIELD_TYPES = new Set(['email', 'password', 'richtext', 'string', 'text', 'uid']);
const IMPLICIT_ATTRIBUTES = new Set(['createdAt', 'updatedAt', 'publishedAt', 'documentId', 'locale']);
...
function getRequiredFieldNames(contentType) {
  return Object.entries(contentType.attributes || {})
    .filter(([, attribute]) => attribute.required && REQUIRED_FIELD_TYPES.has(attribute.type))
    .map(([name, attribute]) => ({
      name,
...
async function getGrowthSeries(strapi, collectionTypes, range) {
  const config = getGrowthConfig(range);
  const buckets = Array.from({ length: config.bucketCount }, (_, index) => {
    const start = config.increment(config.start, index);
    const end = config.increment(start, 1);

    return {
      start,
      end,
      label: formatBucketLabel(start, config.bucketType),
      count: 0,
      documents: new Set(),
    };
  });
...
async function getCollectionHealth(strapi, collection) {
  ...
  const [staleDrafts, staleContent, missingRequiredFields] = await Promise.all([
    supportsPublishedState && hasUpdatedAt
      ? countByQuery(strapi, collection.uid, {
...
async function getContentHealth(strapi, collections) {
  ...
  const collectionHealth = settledHealth
  ...
  return {
    ...
    collections: collectionHealth
      .filter((collection) => collection.issueCount > 0)
...
const service = ({ strapi }) => ({
  async getSummary(rangeKey = DEFAULT_RANGE) {
    ...
    const collectionTypes = Object.entries(strapi.contentTypes || {})
      .filter(([uid, contentType]) => isVisibleCollectionType(uid, contentType))
      .sort(([, first], [, second]) => getDisplayName(first).localeCompare(getDisplayName(second)));
    ...
    const collections = settledMetrics
      .filter((result) => result.status === 'fulfilled')
      .map((result) => result.value)
